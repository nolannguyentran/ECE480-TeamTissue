...
class DataPlotFrame(wx.Frame):
    def __init__(self, motor_name, test_type, strain_type):
        ...
        t_2 = wx.StaticText(panel_2, label = self.motor_name+ " - "+ self.test_type+": "+self.strain_type+" Test Results") 
        t_2.SetFont(font_1)
        t_2.SetForegroundColour((255, 255, 255))
        t_3 = wx.StaticText(panel_4, label = "Please press the 'Home' button to start a new test!")
        t_3.SetFont(font_2)
        ...
        middle_sizer.Add(canvas, 1, wx.EXPAND)
        ...
        self.Layout()
    def on_close():     #function to close the graph
        plt.close(figure)
