...
class ConstantStrainTestInput(wx.Frame):
    def __init__(self, motor_name, test_type, strain_type):
        ...
        strain_text = wx.StaticText(panel_3, label = "Linear Displacement Value (millimeters):\nValue must be between 0 mm to 38.1 mm!")
        ...
        button_start.Bind(wx.EVT_BUTTON, lambda event: gui_be.on_start_test_click(event, self.motor_name, self.test_type, self.strain_type, 0, strain_input.GetValue(), time_input.GetValue(), self.__class__.__name__))
