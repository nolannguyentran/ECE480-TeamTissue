...
class WaveStrainTestInput(wx.Frame):
    def __init__(self, motor_name, test_type, strain_type):
        ...
        button_start.Bind(wx.EVT_BUTTON, lambda event: back_end.on_start_test_click(event, self.motor_name, self.test_type, self.strain_type, wave_strain_min.GetValue(), wave_strain_max.GetValue(), time_input.GetValue(), self.__class__.__name__))
